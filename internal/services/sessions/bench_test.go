...
func BenchmarkService_Create(b *testing.B) {
	...
	for range b.N {
		_, err := svc.Create(ctx, 1, "127.0.0.1", "Benchmark Agent", 24*time.Hour)
		if err != nil {
...
func BenchmarkService_GetByToken(b *testing.B) {
	...
	session, err := svc.Create(ctx, 1, "127.0.0.1", "Benchmark Agent", 24*time.Hour)
	if err != nil {
		b.Fatalf("Failed to create session: %v", err)
	}
