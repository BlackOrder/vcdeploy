...
func (s *MemoryStore) executeUserOp(tx *sql.Tx, op WriteOp) error {
	...
	case WriteOpDelete:
		// Accept both int64 and map[string]string{"id": ...} formats
		var id string
...
func (s *MemoryStore) executeAPIKeyOp(tx *sql.Tx, op WriteOp) error {
	...
	case WriteOpDelete:
		// Support both direct int64 and map[string]string{"id": ...} formats
		var id string
		switch v := op.Data.(type) {
		case string:
			id = v
...
func (s *MemoryStore) executeProjectOp(tx *sql.Tx, op WriteOp) error {
	...
	case WriteOpInsert:
		...
		_, err := tx.Exec(`
			INSERT INTO projects (id, name, repository, branch, deploy_path, type_id, created_at, updated_at, 
				last_deploy_at, last_deploy_status, health_check_id, auto_rollback_enabled, rollback_on_health_fail)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		`, p.ID, p.Name, p.Repository, p.Branch, p.DeployPath, p.TypeID, p.CreatedAt, p.UpdatedAt,
	...
	case WriteOpUpdate:
		...
		_, err := tx.Exec(`
			...
			WHERE id = ?
		`, p.Name, p.Repository, p.Branch, p.DeployPath, p.TypeID, p.UpdatedAt,
